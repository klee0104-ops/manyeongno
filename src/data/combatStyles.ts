...
const legacyCombatStyle=(id:string):CombatStyle=>COMBAT_STYLES[id]??(()=>{const d=spiritById(id);return {weapon:d.role==='수호'?'수호의 방패':d.role==='치유'?'생명의 기운':d.element==='금'?'영혼의 칼날':d.element==='수'?'물결의 술법':d.element==='화'?'불꽃의 일격':'정령의 힘',motion:d.role==='수호'?'전열로 나아가 단단하게 받아칩니다.':d.role==='치유'?'치유의 빛을 동료에게 전합니다.':d.role==='공격'?'적에게 파고들어 일격을 가합니다.':'오행의 투사체를 날립니다.',effect:d.role==='치유'?'leaf':d.role==='수호'?'stone':({목:'venom',화:'flame',토:'club',금:'blade',수:'tide'}[d.element])};})();

export const combatStyle=(id:string):CombatStyle=>{const legacy=legacyCombatStyle(id),p=CHOREOGRAPHIES[id];if(!p)return legacy;return {...legacy,weapon:p.weapon,motion:p.delivery==='melee'?`${p.weapon}을 들고 상대 앞까지 접근해 타격한 뒤 제자리로 돌아옵니다.`:p.delivery==='ground'?`${p.weapon}을 세우고 상대의 발밑에 힘을 펼칩니다.`:p.delivery==='beam'?`${p.weapon}의 힘을 모아 제자리에서 기운을 내보냅니다.`:`${p.weapon}으로 제자리에서 기술을 보내고 동료를 보살핍니다.`};};
