...
import journeyScenes from '../data/journeyScenes.json';
import { canWatchStory, storyLabel, type StoryPart } from '../systems/journey';
export type { StoryPart } from '../systems/journey';
...
export const storyShots=(chapter:number,part:StoryPart)=>{const i=Math.max(0,Math.min(19,chapter-1));return part==='prologue'?journeyScenes.prologue:part==='intro'||part==='outro'?scenes[i][part]:journeyScenes.chapters[i][part];};
...
export function storyFilm(chapter:number,part:StoryPart,onDone:()=>void){
 ...
 overlay.innerHTML=`<header class="theater-header"><div><span>萬靈路 · 이야기의 조각</span><h2>${part==='prologue'?'프롤로그 · 다시 열린 길':`제 ${chapter}장 · ${c.title} · ${storyLabel(part)}`}</h2></div><button data-story="skip">감상 끝내기 ×</button></header><div class="theater-stage"></div><div class="theater-subtitles" aria-live="polite"><small>${c.region} · ${storyLabel(part)}</small><p></p></div><footer class="theater-footer"><div class="story-transport"><button data-story="prev" aria-label="이전 장면">←</button><button data-story="pause">일시정지</button><button data-story="next" aria-label="다음 장면">→</button></div><div class="story-timeline" aria-label="장면 선택">${shots.map((_,i)=>`<button data-story="scene" data-index="${i}" aria-label="${i+1}번째 장면"><i></i></button>`).join('')}</div><div class="narration-control"><button data-story="voice" aria-pressed="true">내레이션 켜짐</button><button data-story="retry" hidden>음성 재생</button><small class="narration-status" role="status"></small></div></footer>`;
...
export function journeyLoading(chapter:number,onWatch:()=>void,onExplore:()=>void,prologue=false){
 ...
 overlay.innerHTML=`<div class="loader-landscape" style="${storyPlate('forest',c.theme)}"></div><div class="loader-shade"></div><div class="loader-content"><span class="loader-seal">緣</span><span class="eyebrow">THE NEXT CHAPTER</span><h1>${c.title}</h1><p>${c.region}의 인연을 불러오고 있습니다.</p><div class="loader-progress" role="progressbar" aria-label="여정 로딩" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><i></i></div><small class="loader-status" role="status">0% · 이야기와 소리를 준비하는 중</small><div class="loader-choice" hidden><button class="primary-button" data-loading="watch">▷ ${prologue?'프롤로그 감상하기':'스토리 감상하기'}</button><button class="outline-button" data-loading="explore">바로 탐험하기 →</button><small>한국어 내레이션 · 자막 · 장면별 2D 연출<br>이야기는 탐험 화면에서 다시 감상할 수 있어요.</small></div></div>`;
 document.body.append(overlay);document.body.classList.add('film-open');let done=false,loaded=0,missing=0;
 const assets=[...['art/story-worlds-v05.png','art/story-relics-v05.png','art/library-v05.png'].map(path=>assetPath(path)),...storyShots(chapter,prologue?'prologue':'intro').map(s=>narrationPath(s.id))];
 const ready=async()=>{
...
export function storyArchive(s:GameSave){
 ...
 return `<div class="modal-heading"><span class="eyebrow">MEMORIES OF THE JOURNEY</span><h2>이야기 감상하기</h2><p>목소리와 장면으로 다시 만나는, 길 위의 인연.</p></div><button class="outline-button full-width" data-action="watch-story" data-id="1:prologue">프롤로그 · 다시 열린 길</button><div class="story-archive">${CAMPAIGN.map((c,i)=>`<article class="${i+1>frontier?'locked':''}"><div class="story-chapter-thumbnail" style="${storyPlate('forest',c.theme)}"><span>${String(i+1).padStart(2,'0')}</span></div><div><small>${c.region}</small><h3>${c.title}</h3><div><button class="outline-button" data-action="watch-story" data-id="${i+1}:intro" ${i+1>frontier?'disabled':''}>${i+1>frontier?'미해금':'서막 감상'}</button>${(['mid5','mid10','mid15'] as const).map(part=>`<button class="outline-button" data-action="watch-story" data-id="${i+1}:${part}" ${canWatchStory(s,i+1,part)?'':'disabled'}>${part.slice(3)}단계 · ${storyLabel(part)}</button>`).join('')}<button class="outline-button" data-action="watch-story" data-id="${i+1}:outro" ${!rating(s,i+1,20,'쉬움')?'disabled':''}>${rating(s,i+1,20,'쉬움')?'종막 감상':'보스 정화 후'}</button></div></div></article>`).join('')}</div>`;
}
